...
# NY Knicks Store Microservices Health Check
echo "🔍 Checking NY Knicks Store Microservices Health..."
echo ""
...
if pg_isready -h localhost -p 5432 >/dev/null 2>&1; then
    PRODUCT_COUNT_DB=$(psql -U postgres -d knicks_auth -t -c "SELECT COUNT(*) FROM products;" 2>/dev/null | xargs)
...
if ! curl -s http://localhost:3000 >/dev/null 2>&1 || ! curl -s http://localhost:3003/products >/dev/null 2>&1; then
    ...
    echo "   3. Or use: docker compose up -d --build"
fi